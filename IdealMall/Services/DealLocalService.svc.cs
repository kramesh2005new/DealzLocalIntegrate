...
namespace IdealMall.Services
{
    ...
    public class DealLocalService : IDealLocalService
    {
        ...
        {
            LocalShopsOffers objLocalShopOffer = PublicBA.GetPublicOffers(new PublicOffersFilterCriteria() { Pagesize = pagesize, Product = product, ShopId = shopID, PageIndex = pageIndex, PostCode = postCode, Radius = string.IsNullOrWhiteSpace(radius) ? Convert.ToDouble(radius) : 0 });
            if (objLocalShopOffer != null)
            {
                if (objLocalShopOffer.AllLocalShops != null && objLocalShopOffer.AllLocalShops.Count > 0)
                {
        ...
        public ShoppingListPublic GetPublicShoppingList(string username)
        {
            ...
            List<LocalShop> lstLocalShops = PublicShoppingBA.GetLocalShops(postcode);

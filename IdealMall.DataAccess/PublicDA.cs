...
namespace IdealMall.DataAccess
{
    ...
    public static class PublicDA
    {
        ...
        public static LocalShopsOffers GetPublicOffers(PublicOffersFilterCriteria filter)
        {
            ...
            else if (!string.IsNullOrEmpty(filter.PostCode) && filter.ShopId != 0)
                return GetPublicOffersBasedOnProductAndShop(filter);
            else if (!string.IsNullOrEmpty(filter.PostCode))
                return GetPublicOffersBasedonProductAndPostCode(filter);
            else if (!string.IsNullOrEmpty(filter.Product))
                return GetPublicOffersBasedOnProductAndShop(filter);
            else if (filter.ShopId != 0)
                return GetPublicOffersBasedOnProductAndShop(filter);
            else
                return new DynamicORM().GetEntity<LocalShopsOffers>(new { filter.PageIndex, filter.Pagesize });
            ...
            try
            {
                using (var db = new IdealmallEntities())
                {
                    ...
                    var offersList = pubOffers.Skip((filterCriteria.PageIndex - 1) * filterCriteria.Pagesize)
                        ...
                        .ToList();
                    lso = ConvertToLocalShopOffersList(offersList);
        ...
        public static LocalShopsOffers GetPublicOffersBasedonProductAndPostCode(
            PublicOffersFilterCriteria filterCriteria)
        {
            var lso = new LocalShopsOffers();

            try
            {
                var latLng = GoogleMapsApiHelper.GetLatLng(filterCriteria.PostCode + ",UK");
                if (false == string.IsNullOrWhiteSpace(filterCriteria.PostCode) &&
                    (false == string.IsNullOrWhiteSpace(filterCriteria.Product) || filterCriteria.ShopId == 0))
                {
                    using (var db = new IdealmallEntities())
                    {
                        if (latLng != null && latLng.Latitude == 0 && latLng.Longitude == 0)
                        {
                            var postCodes = PublicDA.GetPostCodesLike(filterCriteria.PostCode);
                        ...
                        var postCodesInMiles = (from spc in db.SmallPostcodeTables
                                                select new
                                                       {
                                                           postCode = spc.postcode,
                                                           spcLat = spc.latitude,
                                                           spcLng = spc.longitude,
                                                       }).AsEnumerable().Select(s => new
                                                                 {
                                                                     postCode = s.postCode,
                                                                     miles =
                                                                         GetDistanceInMiles(
                                                                             Convert.ToDouble(latLng.Latitude),
                                                                             Convert.ToDouble(latLng.Longitude),
                                                                             Convert.ToDouble(s.spcLat),
                                                                             Convert.ToDouble(s.spcLng))
                                                                 }).Where(w => w.miles < filterCriteria.Radius).ToList();


                        var pubOffers = (from opp in db.OfferProducts_Public
                                         join rts in db.Retailshops
                                             on opp.Shop_Id equals rts.Shop_Id
                                         where
                                             (opp.Product.StartsWith(filterCriteria.Product) ||
                                              opp.Category.StartsWith(filterCriteria.Product) ||
                                              opp.Department.StartsWith(filterCriteria.Product))
                                             && rts.Available == "Y"
                                         select new ProductOfferPublic
                                                {
                                                    ShopId = opp.Shop_Id ?? 0,
                                                    ShopName = rts.Shop_Name,
                                                    ShopImgUrl = rts.Shop_ImageURL,
                                                    ShopPostCode = rts.Postcode,
                                                    ProductName = opp.Product,
                                                    OfferRate = opp.Offer_Rate ?? 0,
                                                    Volume = opp.Volume,
                                                    SellPrice = opp.SellAt ?? 0,
                        ...
                        var pubOffersWithinRadius = (from p in pubOffers
                                                     join pm in postCodesInMiles
                                                         on p.ShopPostCode equals pm.postCode
                                                     orderby pm.miles
                                                     select p).ToList();

                        var offersList = pubOffersWithinRadius.Skip((filterCriteria.PageIndex - 1) * filterCriteria.Pagesize)
                                    .Take(filterCriteria.Pagesize)
                                    .ToList();
                        lso = ConvertToLocalShopOffersList(offersList);
                        lso.Totalcount = pubOffers.Count;

                        //lso = ConvertToLocalShopOffersList(pubOffersWithinRadius);
